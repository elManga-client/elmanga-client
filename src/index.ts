...
ipcMain.on('test-action', async (event, { message }) => {
  ...
  for (let i = 0; i < 3; ++i) {
    try {
      ...
      const serverBaseUrl = await getServerUrl(chapter.id);
      const pageUrls: string[] = [];
      for (const fileName of chapter.attributes.data) {
        pageUrls.push(await getPageUrl(chapter, serverBaseUrl, fileName));
      }
      return event.reply('test-reply', { manga, pageUrls });
    } catch (error) {
      event.reply('test-reply', { error: error.message });
    }
  }
});
